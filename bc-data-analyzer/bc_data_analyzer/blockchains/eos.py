...
class EOS(Blockchain):
    def __init__(self):
        ...
        self.accounts = json.loads(pkgutil.get_data(
            settings.PACKAGE_NAME, "data/eos-accounts.json"))
    ...
    def _output_top_actions_table(self, actions: dict) -> str:
        def format_account(account_actions):
            ...
            actions = [a for a in account_actions["Names"]["Actions"][:3]
            ...
            rows = [f"{multirow(name)} & {multirow(description)} & "
